...
data {
  ...
  int N_outcome_control[(N_obs-1),4];
...
transformed parameters {
  ...
  real<lower=0, upper=1> pred_control_I_S[N];     // predicted total proportion infected with S among AS by time t (cumulative)
  real<lower=0, upper=1> pred_control_I_R[N];     // predicted total proportion infected with R among AS by time t (cumulative)
  real<lower=0, upper=1> pred_control_uninf[N];   // predicted proportion not infected among AS by time t (and are at risk in the next time step)
  real<lower=0, upper=1> pred_control_I_S_new[N]; // predicted proportion of new infections with S among AS group between the interval t and t-1 
  real<lower=0, upper=1> pred_control_I_R_new[N]; // predicted proportion of new infections with R among AS group between the interval t and t-1 
  real<lower=0, upper=1> pred_control_I_undeterm_new[N]; // predicted proportion of new infections with undetermined resistance among AS group between the interval t and t-1 
  real<lower=0, upper=1> pred_control_I_undeterm[N]; 
  ...
  pred_control_I_S[1]=0; 
  pred_control_I_R[1]=0;
  pred_control_I_S_new[1]=0;
  pred_control_I_R_new[1]=0;
  pred_control_I_undeterm_new[1]=0;
  pred_control_I_undeterm[1]=0;
  ...
  for(i in 2:N) {
    ...
    pred_control_I_S_new[i]=  (pred_control_uninf[i-1]*prob_inf*(1-freq))*prob_determ;  // proportion of new infections with S in AS group between time t and t-1
    pred_control_I_R_new[i]=  (pred_control_uninf[i-1]*prob_inf*(freq))*prob_determ;     // proportion of new infections with R in AS group between time t and t-1
    pred_control_I_undeterm_new[i]= ((pred_control_uninf[i-1]*prob_inf*(1-freq)) + (pred_control_uninf[i-1]*prob_inf*(freq)))*(1-prob_determ);
    
    
    pred_control_I_S[i]= pred_control_I_S[i-1]+ pred_control_I_S_new[i];  // proportion of total cumul infections with S in AS group by time t
    pred_control_I_R[i]= pred_control_I_R[i-1]+ pred_control_I_R_new[i];     // proportion of total cumul infections with R in AS group by time t
    pred_control_uninf[i]=pred_control_uninf[i-1] -  pred_control_I_S_new[i] - pred_control_I_R_new[i] - pred_control_I_undeterm_new[i] ;        // proportion without an infection in AS group by time t
    
    pred_control_I_undeterm[i]=pred_control_I_undeterm[i-1] + pred_control_I_undeterm_new[i];    
    }
...
matrix[(N_obs-1),4] probabilities_multi_control;
...
probabilities_multi_control[j-1,1]=(pred_control_I_R[row_model[j]]- pred_control_I_R[row_model[j-1]])/pred_control_uninf[row_model[j-1]];
probabilities_multi_control[j-1,2]=(pred_control_I_S[row_model[j]]- pred_control_I_S[row_model[j-1]])/pred_control_uninf[row_model[j-1]];
probabilities_multi_control[j-1,3]=pred_control_uninf[row_model[j]]/pred_control_uninf[row_model[j-1]];
probabilities_multi_control[j-1,4]=(pred_control_I_undeterm[row_model[j]]- pred_control_I_undeterm[row_model[j-1]])/pred_control_uninf[row_model[j-1]];
