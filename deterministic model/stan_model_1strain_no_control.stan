...
transformed parameters {
  ...
  for(i in 2:N) {
    ...
    pred_treated_I_new[i]=  (pred_treated_uninf[i-1]*prob_inf*(1-p_protect[i])); // proportion of new infections in treated group between time t and t-1
    pred_treated_I[i]= pred_treated_I[i-1]+ pred_treated_I_new[i];               // proportion of total cumul infections in treated group by time t
