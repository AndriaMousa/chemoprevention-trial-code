...
input_1000_dfs<-readRDS("simulated_data/input_df_sim_20days_10ippy_prev0.4_seas_start_N600_200AS_63d_ltf0.1.RData")
...
for (run in 1:N_sims) {  ###R may crash after~500 simulations. I would recommend doing this in batches of no more than 500 sims or run on a HPC
  ...
  N_outcome_control<-as.matrix(cbind(input_df$N_control_I_new[2:nrow(input_df)],
  ...
  stan_data <- list( N_obs = nrow(input_df),                        ## number of observed time points
                     ...
                     N=N,
                     dt=dt)
 model <- stan("stan_model_1strain_with_control.stan",
               data = stan_data,
               chains = 4,
               iter = 5000)
 ...
 params<-rstan::extract( model,pars=c("inc", "lambda", "w"))
  ...
  saveRDS(params,paste0("stan_output/input_df_sim_20days_10ippy_prev0.4_seas_start_N600_200AS_63d_ltf0.1/stan_output_run",run,".rds"))
