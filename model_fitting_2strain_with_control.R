...
input_1000_dfs<-readRDS("simulated_data/input_df_sim_freq0.5_R18days_10ippy_prev0.4_determ0.9_N400_200AS_63d_ltf0.1.RData")
...
for (run in 1:N_sims) {   ###R may crash after~500 simulations. I would recommend doing this in batches of no more than 500 sims or run on a HPC
  ...
  N_outcome_control<-as.matrix(cbind(input_df$N_control_I_R_new[2:nrow(input_df)],
                                     input_df$N_control_I_S_new[2:nrow(input_df)],
                                     input_df$N_control_uninf[2:nrow(input_df)], 
                                     input_df$N_control_I_undeterm_new[2:nrow(input_df)]))
  #stan data list                                                  
  ...
  stan_data <- list( N_obs = nrow(input_df),                        ## number of observed time points
                     ...
                     N_outcome=N_outcome,
                     N_outcome_control=N_outcome_control,
                     N=N,
                     dt=dt)
 model <- stan("stan_model_2strain_with_control.stan",
               data = stan_data,
               chains = 4,
               iter = 5000)
 ...
 params<-rstan::extract( model,pars=c("inc", "freq","lambda_R", "lambda_S", "w_R", "w_S","prob_determ"))
  ...
  saveRDS(params,paste0("stan_output/freq0.5_R18days_10ippy_prev0.4_determ0.9_N400_200AS_63d_ltf0.1/stan_output_run",run,".rds"))
